...
def create_snapshot(repo, commit, diffkemp, functions, output_dir):
    ...
    subprocess.check_call(
        ["make", "prepare", "EXTRA_CFLAGS=-w -fno-pie -no-pie", "HOSTLDFLAGS=-w -no-pie"],
        **kargs
    )
    subprocess.check_call(
        ["make", "modules_prepare", "EXTRA_CFLAGS=-w -fno-pie -no-pie", "HOSTLDFLAGS=-w -no-pie"],
        **kargs
    )
    ...
    for function in functions:
        if f"{function}: unknown" in output:
            ...
            diff_type = "err"
              ...
              f"{function}.diff" in os.listdir(result_path)):
            diff_type = "neq"
        else:
            diff_type = "eq"
